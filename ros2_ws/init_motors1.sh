...
DEFAULT_WS_DIR="$SCRIPT_DIR"
ENV_WS_DIR="${ROS2_WS:-}"
FALLBACK_HOME_WS="/home/elwg/Biolab/ros2_ws"
CLI_WS_DIR=""
WS_DIR=""
SETUP_DIR=""
...
resolve_workspace() {
    local setup_rel="src/openarm_can/setup"
    local candidate
    for candidate in "$WS_DIR" "$DEFAULT_WS_DIR" "$(pwd)"; do
        [ -n "$candidate" ] || continue
        if [ -d "$candidate/$setup_rel" ]; then
            if [ "$WS_DIR" != "$candidate" ]; then
                print_warning "工作空间 '$WS_DIR' 不可用，自动回退到 '$candidate'"
            fi
            WS_DIR="$candidate"
            SETUP_DIR="$WS_DIR/$setup_rel"
            return
        fi
    done
...
usage() {
    ...
    echo "  --ws <path>     指定 ROS2 工作空间目录（覆盖 ROS2_WS）"
...
if [ -n "$CLI_WS_DIR" ]; then
    WS_DIR="$CLI_WS_DIR"
elif [ -n "$ENV_WS_DIR" ]; then
    WS_DIR="$ENV_WS_DIR"
else
    WS_DIR="$FALLBACK_HOME_WS"
fi
