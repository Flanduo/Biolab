...
std::vector<double> parse_double_list(const std::string& s,
  ...
  std::string item;
  size_t i = 0;
  while (std::getline(ss, item, ',') && i < expected_size) {
    try {
...
bool OpenArm_v10HW::parse_config(const hardware_interface::HardwareInfo& info) {
  ...
  it = info.hardware_parameters.find("enable_tau_ff_test");
  if (it != info.hardware_parameters.end()) {
    std::string value = it->second;
    std::transform(value.begin(), value.end(), value.begin(), ::tolower);
    enable_tau_ff_test_ = (value == "true");
  }

  it = info.hardware_parameters.find("tau_ff_test_offsets");
  if (it != info.hardware_parameters.end()) {
    tau_ff_test_offsets_ = parse_double_list(it->second, ARM_DOF);
  }

  it = info.hardware_parameters.find("tau_ff_limits");
  if (it != info.hardware_parameters.end()) {
    tau_ff_limits_ = parse_double_list(it->second, ARM_DOF);
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
              "tau_ff test: %s | offsets=[%s] | limits=[%s]",
              enable_tau_ff_test_ ? "enabled" : "disabled",
              format_double_list(tau_ff_test_offsets_).c_str(),
              format_double_list(tau_ff_limits_).c_str());
  return true;
}
...
hardware_interface::return_type OpenArm_v10HW::write(
  ...
  std::vector<double> extra_ff(ARM_DOF, 0.0);
  if (enable_tau_ff_test_) {
    for (size_t i = 0; i < ARM_DOF && i < tau_ff_test_offsets_.size(); ++i) {
      extra_ff[i] = tau_ff_test_offsets_[i];
    }
  }

  std::vector<openarm::damiao_motor::MITParam> arm_params;
  for (size_t i = 0; i < ARM_DOF; ++i) {
    double tau_ff = tau_commands_[i] + extra_ff[i];
    if (i < tau_ff_limits_.size()) {
      tau_ff = std::clamp(tau_ff, -tau_ff_limits_[i], tau_ff_limits_[i]);
    }
