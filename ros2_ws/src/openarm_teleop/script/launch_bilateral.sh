...
# Try to auto-detect workspace directory
...
if [ ! -d "$WS_DIR" ]; then
    ...
    echo "Please update WS_DIR in this launch script or ensure the workspace is in the expected location." >&2
    exit 1
fi
...
echo "[INFO] Xacro file: $XACRO_PATH"
echo "[INFO] Arm side: $ARM_SIDE"
echo "[INFO] Leader CAN interface: $LEADER_CAN_IF"
echo "[INFO] Follower CAN interface: $FOLLOWER_CAN_IF"
mkdir -p "$TMPDIR"
...
echo "[INFO] URDFs generated successfully:"
echo "[INFO]   Leader URDF: $LEADER_URDF_PATH"
echo "[INFO]   Follower URDF: $FOLLOWER_URDF_PATH"
