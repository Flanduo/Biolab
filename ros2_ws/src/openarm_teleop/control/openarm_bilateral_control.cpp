...
public:
    AdminThread(std::shared_ptr<RobotSystemState> leader_state,
                ...
                Control *control_f, double hz = 1000.0)
...
int main(int argc, char **argv) {
    try {
        ...
        // Set peer state for direct reference setting (leader->follower, zero delay)
        control_leader->SetPeerState(follower_state);
        ...
        // Note: AdminThread is no longer needed - Leader directly sets Follower's reference in bilateral_step()
