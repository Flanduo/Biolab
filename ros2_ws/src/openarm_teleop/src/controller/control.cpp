...
bool Control::bilateral_step() {
    ...
    // For leader: directly set follower's reference to current position (zero delay, direct mapping)
    // 主臂怎么动，从臂就怎么动 - 直接映射，无延迟
    if (role_ == ROLE_LEADER && peer_state_ != nullptr) {
        peer_state_->arm_state().set_all_references(joint_arm_states);
        peer_state_->hand_state().set_all_references(joint_gripper_states);
    }
    ...
    if (role_ == ROLE_LEADER) {
        ...
        joint_gripper_states_ref = joint_gripper_states;
    } else {
        // 跟随臂正常跟随参考
        joint_arm_states_ref = robot_state_->arm_state().get_all_references();
        joint_gripper_states_ref = robot_state_->hand_state().get_all_references();
    }
    
    // For left arm, reverse gripper direction (mirror effect)
    if (arm_type_ == "left_arm") {
        for (size_t i = 0; i < joint_gripper_states_ref.size(); ++i) {
            joint_gripper_states_ref[i].position = -joint_gripper_states_ref[i].position;
            joint_gripper_states_ref[i].velocity = -joint_gripper_states_ref[i].velocity;
        }
    }
    ...
    for (size_t i = 0; i < gripper_dof; i++) {
        // For left arm gripper, reverse friction compensation direction to match reversed position/velocity
        // This ensures consistent behavior between left and right arms
        if (arm_type_ == "left_arm") {
    ...
    // For leader (zero force feedback), use very low Kp/Kd for gripper to allow free movement
    std::vector<openarm::damiao_motor::MITParam> gripper_cmds;
    gripper_cmds.reserve(gripper_dof);
    for (size_t i = 0; i < gripper_dof; ++i) {
        double gripper_kp = Kp_[i + arm_dof];
        double gripper_kd = Kd_[i + arm_dof];
        // For leader, reduce gripper Kp/Kd significantly for zero force feedback
        // Use same low values for both left and right arms to ensure consistent feel
        if (role_ == ROLE_LEADER) {
            gripper_kp = 0.2;  // Very low Kp for leader gripper (consistent for both arms, more compliant)
            gripper_kd = 0.005; // Very low Kd for leader gripper (consistent for both arms, more compliant)
        }
        gripper_cmds.emplace_back(openarm::damiao_motor::MITParam{
            gripper_kp, gripper_kd, motor_gripper_states[i].position,
    ...
    // 减少延迟：移除不必要的sleep，直接接收数据
    // 对于CAN-FD，通信延迟通常<100微秒，不需要200微秒的等待
