...
class Control {
    ...
    std::shared_ptr<RobotSystemState> peer_state_;  // For direct reference setting (leader->follower)
...
public:
    ...
    // Set peer state for direct reference setting (leader->follower, no delay)
    void SetPeerState(std::shared_ptr<RobotSystemState> peer_state) {
