...
bool RobStrideMotor::receive_status_frame(double timeout_sec) {
  auto result = receive(timeout_sec);
  if (!result) {
    // 超时或接收失败，不抛出异常，返回 false
    return false;
  }
...
void RobStrideMotor::Set_RobStrite_Motor_parameter(uint16_t Index, float Value,
  ...
  receive_status_frame(0.01);  // 10ms 超时
}
...
uint16_t RobStrideMotor::float_to_uint(float x, float x_min, float x_max,
  ...
  if (span <= 0.0f) {
    // 防止除零错误
    return static_cast<uint16_t>((1 << (bits - 1)));  // 返回中点值
  }
...
RobStrideMotor::send_motion_command(float torque, float position_rad,
  ...
  if (n < 0) {
    // 写入失败，可能是CAN总线错误
    perror("send_motion_command write failed");
    throw std::runtime_error("Failed to write CAN frame");
  } else if (n != sizeof(frame)) {
    // 部分写入，不应该发生
    std::cerr << "Warning: send_motion_command partial write: " << n 
              << " bytes instead of " << sizeof(frame) << std::endl;
  }
  // 尝试接收状态帧，如果失败也不影响继续运行
  receive_status_frame(0.01);  // 10ms 超时
  return std::make_tuple(position_, velocity_, torque_, temperature_);
}
